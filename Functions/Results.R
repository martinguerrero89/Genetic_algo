...
R=PVAL$Genes[[generations]]
k=PVAL$k[generations]

save(file=paste0(resultdir,"results.rda"),PVAL)

#Calcular centroides del dataset  completo

hsp_class=mycluster(t(prob_matrix[R,]),k)

C=kcentroid(prob_matrix[R,],hsp_class$cluster)

write.table(C,paste0(resultdir,"centroids.txt"),sep="\t",col.names =LETTERS[1:k],row.names=TRUE)
hsp_class= classify(prob_matrix[R,],C)
##

hsp_classdf= as.data.frame(hsp_class)
hspClinic= cbind(clinical,hsp_classdf)
...
#  attach(hspClinic)
#  mysurv <- Surv(OS, vital_status==vit_stat)
#  tumortotal <- survfit(mysurv~ hsp_class)
#  totalsdf <- survdiff(mysurv~ hsp_class)
#  tumortotalpval <- 1 - pchisq(totalsdf$chisq, length(totalsdf$n) - 1)
#  OVERFIT[i]=tumortotalpval
#}
